...
<section id="content">
  <div class="container">
    ...
    <center><div id="record">
    <!-- <form action="/schedule" method="post">  -->
    
            <form action="/lawyer_doc_get" enctype="multipart/form-data" method="POST">
          @csrf
      
      <!-- <input type="text" name="email" value=""> -->
            <table>
            Client<select id="doc_email" name="doc_email">
      ...
      @foreach($u as $key => $email)
         ...
         Which case<select name="title" required  id="title" >
                        <option >Select case</option>
                    </select><br><br>
            <!-- Title of document<input type="text" name="document" >&nbsp; -->
          <input type="submit" name="submit" value="Search"><br><br>
          <!-- <button type="submit" style="margin-top:5%" class="btn btn-success pull-right" onclick="createPDF()">Download</button> -->
      </form>
...
<script type="text/javascript">
   ...
   // alert(email);   
    if(email){
        $.ajax({
           type:"GET",
           url:"{{url('getlaw-title-list')}}?email="+email,
