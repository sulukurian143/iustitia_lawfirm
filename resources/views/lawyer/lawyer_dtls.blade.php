...
<section id="content">
  <div class="container">
    ...
    <form action="{{route('registrations.show',$a[0]->email)}}" 	>
            @csrf
						<?php
                $email=Session::get('email');
?>
				<div class="wrap-input100 validate-input" data-validate = "Valid email is required: ex@abc.xyz">
					<input type="hidden" name="fname" value="{{ $a[0]->fname }}"/>
					<input type="hidden" name="lname" value="{{ $a[0]->lname }}"/>
					<input type="hidden" name="address" value="{{ $a[0]->address }}"/>
					<input type="hidden" name="type" value="{{ $a[0]->type }}"/>
					<input type="hidden" name="proof" value="{{ $a[0]->proof }}"/>
					<input type="hidden" name="gender" value="{{ $a[0]->gender }}"/>
					<input type="hidden" name="country" value="{{ $a[0]->country }}"/>
					<input type="hidden" name="state" value="{{ $a[0]->state }}"/>
					<input type="hidden" name="city" value="{{ $a[0]->city }}"/>
					<input type="hidden" name="password" value="{{ $a[0]->password }}"/>
					<input type="hidden" name="phone" value="{{ $a[0]->phone }}"/>
					<input type="hidden" name="status" value="{{ $a[0]->status }}"/>
					<input type="hidden" name="approve" value="{{ $a[0]->approve }}"/>
					<b>Email Address</b><input id="email" class="input100" type="text"  name="email" readonly value=<?php echo $email;?>>
...
<br><br>				
				...
				<b>Area of Specialization</b>
						...
						<select class="js-select2" name="category">
							...
							<option>Medical Malpractice Lawyer</option>
                            <option>Family Lawyer</option>
                            <option>Workers Compensation Lawyer</option>
						</select>
						
				<br><br><b>Office</b>
						<select class="js-select2" name="office">
							<option disabled selected>Please choose</option>
