        ...
        <div class="card mb-3">
          ...
          @foreach($data as $a)
             ...
             <!-- <td><input type="text" name="date" disabled value="{{$a->email}} " disabled></td> -->
                <td><input type="text" id="date" name="date" disabled value="{{$a->date}} " disabled></td>
                <td><input type="text" name="b1" disabled value="{{$a->stime}}"></td>
                <td><input type="button" name="b3" id="button{{$a->id}}" value="{{$a->token}}" onclick="myFunction({{$a->id}}, '{{$a->date}}')"></td>
...
function myFunction(id, date) {
  ...
  var data = { 'id': id,'date': date,'_token':$('meta[name="csrf-token"]').attr('content') };
 console.log(id);
 console.log(date);
  ...
  $.ajax({
    ...
    success: function (result) {
      document.getElementById('button'+id).value=result;
     alert(result);
     @foreach($data as $a)
     $("#button{{$a->id}}").attr("disabled", true);
     @endforeach
    }
  });
}
